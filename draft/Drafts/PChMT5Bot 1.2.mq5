 ...
 //   if (lastTradeTime != iTime(NULL, 0, 0)) {
       ...
       //aa calculateRisk();
        ...
        if (isBullishEngulfing(0)) {
            ...
            double entryPrice = iClose(NULL, 0, 0);
            
            double stopLoss = iLow(NULL, 0, 0) * slRatio;
            double takeProfit = entryPrice + 2 * (entryPrice - stopLoss) * tpRatio;
            
            // Place Buy order
            trade.PositionOpen(_Symbol, ORDER_TYPE_BUY, 1, entryPrice, stopLoss, takeProfit, "Bullish Engulfing");
//            OrderSend(Symbol(), OP_BUY, 1, entryPrice, 3, stopLoss, takeProfit, "Bullish Engulfing", 0, 0, Green);
       //     lastTradeTime = iTime(NULL, 0, 0);
        ...
        if (isBearishEngulfing(0)) {
            ...
            double entryPrice = iClose(NULL, 0, 0);
            double stopLoss = iHigh(NULL, 0, 0)* slRatio;
            double takeProfit = entryPrice - 2 * (stopLoss - entryPrice) * tpRatio;

            // Place Sell order
                  trade.PositionOpen(_Symbol, ORDER_TYPE_SELL, 1, entryPrice, stopLoss, takeProfit, "Bearish Engulfing");
